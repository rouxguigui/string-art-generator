<template>
  <div class="board">
    <div class="board-container" :class="boardClass" :style="{zoom: zoom}" :width="convertToPx(board.width) + 40"
         :height="convertToPx(board.height) + 60">
...
export default {
    ...
    methods: {
      ...
      if (stringLayer.settings.pattern === 'custom') {
        lastNail = null;
      }

      for (let loopIndex = 0; loopIndex < loopCount; loopIndex++) {
        if (stringLayer.settings.pattern === 'aqua') {// Aqua
          let nail = this.getNail(startNail + loopIndex);
          this.canvas.context.moveTo(nail.x, nail.y);

          let nail2 = this.getNail(startNail + loopIndex * 2 + this.nails.length / 2);
        ...
        } else if (stringLayer.settings.pattern === 'rectangle')
        {
          ...
          if (index === 0) {
            nail2 = this.getNail(startNail + 34 + 0 + loopIndex);
            this.canvas.context.lineTo(nail2.x, nail2.y);
          } else if (index === 1) {
            nail2 = this.getNail(startNail + 34 + 13 + loopIndex);
            this.canvas.context.lineTo(nail2.x, nail2.y);
          } else if (index === 2) {
            nail2 = this.getNail(startNail + 34 + 27 + loopIndex);
            this.canvas.context.lineTo(nail2.x, nail2.y);
          } else if (index === 3) {
            nail2 = this.getNail(startNail + 34 + 43 + loopIndex);
            this.canvas.context.lineTo(nail2.x, nail2.y);
          } else if (index === 4) {
            nail2 = this.getNail(startNail + 34 + 56 + loopIndex);
            this.canvas.context.lineTo(nail2.x, nail2.y);
          } else if (index === 5) {
            nail2 = this.getNail(startNail + 34 + 66 + loopIndex);
            this.canvas.context.lineTo(nail2.x, nail2.y);
            //} else if (index === 5) {
            //  nail2 = this.getNail(startNail + 34 + 0 + i);
            //  this.canvas.context.lineTo(nail2.x, nail2.y);
            //} else if (index === 6) {
            //  nail2 = this.getNail(startNail - 34 - 13 - i);
            //  this.canvas.context.lineTo(nail2.x, nail2.y);
            //} else {
            //  nail2 = this.getNail(startNail + 34 + 34 - i);
            //  this.canvas.context.lineTo(nail2.x, nail2.y);
          }
          ...
          for (let step of stringLayer.settings.patternSteps) {
            ...
            try {
              let nail = null;
              const layer = this.nailsLayers[step.layer];
              if (!layer) {
                console.warn(`Invalid layer for pattern ${step.layer}`);
                continue;
              }
              let delta = parseInt(step.delta);
              // if (delta) {
                nail = layer.nails[step.nail + delta * loopIndex];
              // } else {
